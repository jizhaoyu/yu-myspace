const KNOWN_ERRORS: Array<{ match: RegExp; message: string }> = [
  ...
  { match: /only queued tasks can be edited/i, message: "仅排队中的任务可以编辑。" },
  { match: /task is not in the queue/i, message: "任务已不在队列中。" },
  { match: /unknown task/i, message: "任务不存在或已被清理。" },
  { match: /batch prompts must not be empty/i, message: "批量提示不能为空。" },
  { match: /missing endpoint, model, or api key/i, message: "请先完成引擎配置（Endpoint、Model、API Key）。" },
  { match: /mcp server id is required/i, message: "MCP 服务 ID 不能为空。" },
  { match: /failed to open path/i, message: "无法打开该路径，请检查权限或路径有效性。" },
